...
/// IP Blacklist Map — written by userspace to drop malicious IPs at wire speed
#[map]
static ip_blacklist_map: HashMap<u32, u8> = HashMap::with_max_entries(65536, 0);
...
fn try_kelan_xdp(ctx: XdpContext) -> Result<u32, ()> {
    ...
    // Check IP Blacklist
    if let Some(blocked) = unsafe { ip_blacklist_map.get(&src_addr) } {
        if *blocked != 0 {
